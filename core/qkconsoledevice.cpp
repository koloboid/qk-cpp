...
QkConsoleDevice::QkConsoleDevice()
{
}

QFile* QkConsoleDevice::stdOut()
{
    if (!mStdOut)
    {
        mStdOut = new QFile();
...
bool QkConsoleDevice::isConsoleDevice(QIODevice *pDevice)
{
    QFile* file = qobject_cast<QFile*>(pDevice);
    return file && (file->handle() == STDOUT_FILENO || file->handle() == STDERR_FILENO);
}

void QkConsoleDevice::setForeColor(const Qt::GlobalColor pColor)
{
    switch (pColor)
    {
        case Qt::black:
